    ...
    for i in range(n):
        ...
        if np.less_equal(np.fabs(pivot), np.finfo(np.float64).eps):
            ...
            for j in range(i+1, n):
                if np.greater(a[j, i], a[ligneValMax, i]):
                    ligneValMax = j
            for k in range(i,n+1):
                tmp = a[i, k-1]
                a[i, k-1] = a[ligneValMax,k-1]
                a[ligneValMax, k-1] = tmp
        if i < n - 1:
            frac = a[i+1:, i] / pivot
...
if __name__ == "__main__":
    a, b, x = systeme_lineaire_non_symetrique(400)

    print("Résolution du système Ax = b... ", end="")
    ...
    print("NUMPY >>> Résolution du système Ax = b... ", end="")
    debut = t.perf_counter()
